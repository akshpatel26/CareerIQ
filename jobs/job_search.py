...
def render_market_insights_individual():
    """Alternative version with individual expanders for each item"""
    insights = get_market_insights()
    
    st.markdown("### 📊 Job Market Insights")
    
    # Individual expanders for trending skills
    st.subheader("🔥 Trending Skills")
    for skill in insights["trending_skills"]:
        with st.expander(f"{skill['name']} - Growth: {skill['growth']}", expanded=False):
            col1, col2 = st.columns([1, 3])
            with col1:
                st.markdown(f'<i class="{skill["icon"]}" style="font-size: 3rem; color: #00bfa5;"></i>', 
                           unsafe_allow_html=True)
            with col2:
                st.write(f"**{skill['name']}** is showing significant growth in the job market.")
                st.write(f"Growth rate: **{skill['growth']}**")
                st.info("This skill is in high demand across multiple industries.")
    
    # Individual expanders for locations
    st.subheader("📍 Top Job Locations")
    for location in insights["top_locations"]:
        with st.expander(f"{location['name']} - {location['jobs']} jobs", expanded=False):
            col1, col2 = st.columns([1, 3])
            with col1:
                st.markdown(f'<i class="{location["icon"]}" style="font-size: 3rem; color: #00bfa5;"></i>', 
                           unsafe_allow_html=True)
            with col2:
                st.write(f"**{location['name']}** is a major tech hub with abundant opportunities.")
                st.write(f"Available positions: **{location['jobs']}**")
                st.success("Strong job market with competitive salaries.")
    
    # Individual expanders for salary insights
    st.subheader("💰 Salary Insights by Role")
    role_icons = {
        "Software Engineer": "fas fa-code",
        "Data Scientist": "fas fa-brain",
        "Product Manager": "fas fa-tasks",
    ...
    for insight in insights["salary_insights"]:
        ...
        with st.expander(f"{role} - ₹{insight['range']}", expanded=False):
            col1, col2 = st.columns([1, 3])
            with col1:
                st.markdown(f'<i class="{icon}" style="font-size: 3rem; color: #00bfa5;"></i>', 
                           unsafe_allow_html=True)
            with col2:
                st.write(f"**Role:** {role}")
                st.write(f"**Salary Range:** ₹{insight['range']}")
                st.write(f"**Experience Required:** {insight['experience']}")
                
                # Additional details
                if "Software Engineer" in role:
                    st.info("High demand for full-stack and backend developers.")
                elif "Data Scientist" in role:
                    st.info("Machine learning and AI skills command premium salaries.")
                elif "Product Manager" in role:
                    st.info("Strong business acumen and technical knowledge required.")
                elif "DevOps Engineer" in role:
                    st.info("Cloud platforms and automation skills are essential.")
                elif "UI/UX Designer" in role:
                    st.info("User-centered design and prototyping skills valued.")
...
if __name__ == "__main__":
    ...
    version = st.radio("Choose display format:", 
                      ["Grouped Expanders", "Individual Expanders"])
    
    if version == "Grouped Expanders":
        render_market_insights()
    else:
        render_market_insights_individual()
